...
#   版本: v18 (2025-11-11 - 時間格式修正版)
#
#   - 修正 v11/v17 的時間格式不統一問題 (混合 HH:MM 和 HH:MM:SS.f)。
#   - 強制所有 'published_at' 和 'scraped_at' 輸出為 'YYYY-MM-DD HH:MM:SS'。
#   - 這能解決 CSV 在 Excel / Numbers 中開啟時，日期格式錯亂的問題。
#   - 保留 v17 的所有特性 (7欄位, News_dataset.csv, 反爬蟲)。
#
# ===============================================================
...
def setup_environment():
    ...
    for pkg_name, import_name in packages:
        try:
            ...
            try:
                # *** 請確認您的 python 執行檔路徑 ***
                python_executable = "/opt/anaconda3/bin/python3"
                ...
                logging.error(f"🔥 嚴重錯誤：找不到 Python 執行檔 '{python_executable}'")
                logging.error("請修改腳本 Cell 2 中的 'python_executable' 變數為您正確的路徑。")
                sys.exit(1)
...
def get_dynamic_user_agent():
    ...
    if ua:
        try:
            return ua.random
        except Exception: # 處理 fake-useragent 可能的連線錯誤
            return 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Safari/537.36'
    else:
        return 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Safari/537.36'

def get_soup(url):
    """輔助函式：發送請求並取得 BeautifulSoup 物件 (v16 強化版)"""
    try:
...
def scrape_article_content_base(url, content_selector, time_selector):
    ...
    published_at_str = re.sub(r'^(發布時間|更新時間)：', '', published_at_str).strip()
    
    # 嘗試匹配 YYYY-MM-DD HH:MM:SS (最優先)
    match = re.search(r'(\d{4}[-/]\d{2}[-/]\d{2}\s+\d{2}:\d{2}:\d{2})', published_at_str)
    if match:
        return clean_content(content), match.group(1).replace('/', '-')

    # 嘗試匹配 YYYY-MM-DD HH:MM
    match = re.search(r'(\d{4}[-/]\d{2}[-/]\d{2}\s+\d{2}:\d{2})', published_at_str)
    if match:
        return clean_content(content), match.group(1).replace('/', '-') + ":00" # 補上秒數

    # 嘗試匹配 YYYY-MM-DD
    match = re.search(r'(\d{4}[-/]\d{2}[-/]\d{2})', published_at_str)
    if match:
        return clean_content(content), match.group(1).replace('/', '-') + " 00:00:00" # 補上時間

    # 備案 2: 如果都找不到，使用當前抓取時間 (標準格式)
    return clean_content(content), get_current_utc_time()
...
def fetch_cna_headlines():
    ...
    if not soup:
        return []
    headlines = []
    scraped_at = get_current_utc_time() # v18: 格式 YYYY-MM-DD HH:MM:SS
...
def fetch_tvbs_headlines():
    ...
    soup = get_soup(base_domain + '/realtime') 
    if not soup:
        return []
    headlines = []
    scraped_at = get_current_utc_time() # v18: 格式 YYYY-MM-DD HH:MM:SS
    
    for item in soup.select('div.realtime-main_art a'):
        try:
            title = item.get('title')
            if not title:
    ...
    for item in soup.select('ul.news-list-new li a'):
        try:
            title_element = item.select_one('h2.title')
            title = title_element.get_text(strip=True) if title_element else item.get('title')
            url = normalize_url(base_domain, item.get('href'))
            if title and url and 'news.tvbs.com.tw' in url:
...
def fetch_ebc_headlines():
    ...
    for item in soup.select('div.newest_news-list a.item'):
        try:
            title = item.select_one('h3.item_title').get_text(strip=True)
            url = normalize_url(base_url, item.get('href'))
            if title and url and 'ebc.net.tw' in url:
...
def fetch_ettoday_headlines():
    ...
    for item in soup.select('div.part_list_2 div.piece.clearfix'):
        try:
            ...
            logging.warning(f"🟡 ETtoday Level 1/3 剖析錯誤: {e}")
...
def scrape_nownews_article(url):
    ...
    if match:
        return clean_content(content), match.group(1).replace('/', '-') + ":00"

    match = re.search(r'(\d{4}[-/]\d{2}[-/]\d{2})', time_str)
    if match:
        return clean_content(content), match.group(1).replace('/', '-') + " 00:00:00"
        ...
        try:
            ...
            if content and content != 'N/A': 
                ...
                new_snapshot_data.append({
                    ...
                    'content': content,
                    'source': source,
                    'published_at': pub_time, # v18: 已標準化為 YYYY-MM-DD HH:MM:SS
                    'scraped_at': headline['scraped_at'], # v18: 已標準化為 YYYY-MM-DD HH:MM:SS
...
if __name__ == "__main__":
    ...
    # 步驟零：安裝套件 (請確保您已安裝 fake-useragent)
    # setup_environment() # 註解掉，因為您應該已經安裝好了

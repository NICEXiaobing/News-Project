 ...
 "cells": [
  {
   "cell_type": "code",
   "execution_count": 2,
   "id": "52c96ffe-6ca9-4005-989d-315d5e16d757",
   "metadata": {},
   "outputs": [
    {
     "name": "stdout",
     "output_type": "stream",
     "text": [
      "2025-11-18 17:51:20 - INFO - ======= V3.8 任務開始 2025-11-18 17:51:20 =======\n",
      "2025-11-18 17:51:20 - INFO - --- [V3.8] Part 1: 抓取 9 大媒體頭條 ---\n",
      "2025-11-18 17:51:21 - INFO -    YAHOO: 17 筆\n",
      "2025-11-18 17:51:22 - INFO -    CNA: 100 筆\n",
      "2025-11-18 17:51:23 - INFO -    UDN: 20 筆\n",
      "2025-11-18 17:51:24 - INFO -    SETN: 136 筆\n",
      "2025-11-18 17:51:26 - INFO -    TVBS: 4 筆\n",
      "2025-11-18 17:51:27 - INFO -    NOWnews: 12 筆\n",
      "2025-11-18 17:51:28 - INFO -    ETtoday: 100 筆\n",
      "2025-11-18 17:51:29 - INFO -    LTN: 20 筆\n",
      "2025-11-18 17:51:31 - INFO -    EBC: 34 筆\n",
      "2025-11-18 17:51:32 - INFO - ✅ Part 1 完成: 總計 424 筆\n",
      "2025-11-18 17:51:32 - INFO - --- [V3.8] Part 2: 抓取內文並存檔 ---\n",
      "2025-11-18 17:54:17 - INFO - ✅ Part 2 完成: 寫入 376 筆 到 /Users/xiaobing/anaconda_projects/News_Projects/news_data_v3_content.csv\n",
      "2025-11-18 17:54:17 - INFO - --- [V3.8] Part 3: Google News 標籤抓取 ---\n",
      "2025-11-18 17:54:38 - INFO - ✅ Part 3 完成: 寫入 1179 筆\n",
      "2025-11-18 17:54:38 - INFO - --- [V3.8] Part 4: 資料合併 ---\n",
      "2025-11-18 17:54:39 - INFO - 🎉 合併完成! 新增 17 筆關聯\n",
      "2025-11-18 17:54:39 - INFO - ======= V3.8 任務結束，耗時 199.46 秒 =======\n"
     ]
    }
   ],
   "source": [
    ...
    "\n",
    "# ===============================================================\n",
    "#   T-Forecast 全自動爬蟲系統 (V3.8 LTN 結構修正版)\n",
    "#   更新日期: 2025-11-18\n",
    "#   \n",
    "#   [本次修復]\n",
    "#   1. LTN (自由): 修正選擇器錯誤。改抓 `ul.list li a` 並直接讀取 title 屬性。\n",
    "#   2. 維持 V3.7 的 Yahoo, EBC, CNA 等穩定邏輯。\n",
    "# ===============================================================\n",
    "\n",
    "import sys\n",
    "import logging\n",
    ...
    "    BASE_DIR = os.getcwd()\n",
    "\n",
    "CONTENT_DB_V3 = os.path.join(BASE_DIR, 'news_data_v3_content.csv')\n",
    "GOOGLE_DB_V3  = os.path.join(BASE_DIR, 'news_data_v3_google.csv')\n",
    "FINAL_DB_V3   = os.path.join(BASE_DIR, 'news_data_v3_final.csv')\n",
    "\n",
    "TEMP_JSON_FILENAME = os.path.join(BASE_DIR, 'temp_headlines_v3.json')\n",
    "LOG_FILENAME = os.path.join(BASE_DIR, 'scraper_v3.log')\n",
    "\n",
    "# --- Logging (防重複版) ---\n",
    ...
    "    retry = Retry(connect=3, backoff_factor=0.5) # 失敗後等待 0.5s 再試，最多 3 次\n",
    ...
    "        logging.warning(f\"🟡 抓取 {url} 失敗 (已重試): {e}\")\n",
    "        return None\n",
    "\n",
    ...
    "\n",
    "# ===============================================================\n",
    "# 3. [Part 1] 頭條抓取 (V3.8 LTN 修正)\n",
    "# ===============================================================\n",
    "\n",
    "def fetch_ltn_headlines():\n",
    "    \"\"\"V3.8: 根據診斷報告修正 LTN 選擇器\"\"\"\n",
    ...
    "    \n",
    "    # 修正: 直接抓 whitecon 下列表的 a 標籤\n",
    "    # 診斷顯示連結本身有 title 屬性，例如 title=\"低溫來了...\"\n",
    "    for item in soup.select('div.whitecon ul.list li a'):\n",
    "        title = item.get('title')\n",
    "        # 如果 title 屬性沒有，再試著抓內部的 h3\n",
    "        if not title:\n",
    ...
    "    if soup_home:\n",
    "        for h3 in soup_home.find_all('h3'):\n",
    "            link_tag = h3.find('a', href=True)\n",
    "            if link_tag:\n",
    "                link = link_tag['href']\n",
    "                title = link_tag.get_text(strip=True)\n",
    "                if title and len(title) > 5 and ('/html' in link or '/news/' in link):\n",
    "                    headlines.append({'url': normalize_url(base_url, link), 'title': title, 'headline_level': 3, 'scraped_at': scraped_at, 'source': 'YAHOO'})\n",
    "\n",
    ...
    "    headlines = []\n",
    "    scraped_at = get_current_utc_time()\n",
    "    soup_home = get_soup(base_domain)\n",
    "    if soup_home:\n",
    "        for item in soup_home.select('div.hero_main a.swiper-slide'):\n",
    ...
    "    logging.info(\"--- [V3.8] Part 1: 抓取 9 大媒體頭條 ---\")\n",
    "    all_headlines = []\n",
    "    \n",
    "    fetchers = [\n",
    ...
    "    logging.info(\"--- [V3.8] Part 2: 抓取內文並存檔 ---\")\n",
    "    try:\n",
    ...
    "        is_exist = os.path.exists(CONTENT_DB_V3)\n",
    "        df.to_csv(CONTENT_DB_V3, mode='a', header=not is_exist, index=False, encoding='utf-8-sig')\n",
    "        logging.info(f\"✅ Part 2 完成: 寫入 {len(new_data)} 筆 到 {CONTENT_DB_V3}\")\n",
    "    else:\n",
    ...
    "    logging.info(\"--- [V3.8] Part 3: Google News 標籤抓取 ---\")\n",
    "    data_list = []\n",
    ...
    "        is_exist = os.path.exists(GOOGLE_DB_V3)\n",
    "        df.to_csv(GOOGLE_DB_V3, mode='a', header=not is_exist, index=False, encoding='utf-8-sig')\n",
    ...
    "    logging.info(\"--- [V3.8] Part 4: 資料合併 ---\")\n",
    "    if not os.path.exists(CONTENT_DB_V3) or not os.path.exists(GOOGLE_DB_V3): return\n",
    "    try:\n",
    "        df_content = pd.read_csv(CONTENT_DB_V3)\n",
    "        df_google = pd.read_csv(GOOGLE_DB_V3)\n",
    ...
    "                match_count += 1\n",
    "        df_content.to_csv(FINAL_DB_V3, index=False, encoding='utf-8-sig')\n",
    ...
    "    logging.info(f\"======= V3.8 任務開始 {get_current_utc_time()} =======\")\n",
    ...
    "    run_part4_merge_v3()\n",
    "    end_time = time.time()\n",
    "    logging.info(f\"======= V3.8 任務結束，耗時 {end_time - start_time:.2f} 秒 =======\")\n",
    "\n",
  ...
  },
  {
   ...
   "id": "850521e8-96c2-4469-97d4-80e3ea99e309",
   "metadata": {},
   "outputs": [],
   "source": []
  }
